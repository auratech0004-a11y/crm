      ...
      <div className="w-full max-w-md bg-card rounded-3xl shadow-2xl shadow-foreground/5 overflow-hidden border border-border animate-scale-in">
        <div className="gradient-primary p-10 text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl font-extrabold text-primary-foreground">CRM A.R HRMS</h2>
            <p className="text-primary-foreground/80 mt-2">Professional HR & Admin System</p>
          </div>
        </div>

        <form onSubmit={handleLogin} className="p-8 space-y-6" data-testid="login-form">
          {error && (
            <div className="bg-destructive/10 text-destructive p-4 rounded-xl text-sm border border-destructive/20 flex items-center gap-3 animate-slide-up" data-testid="login-error">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 gradient-primary text-primary-foreground rounded-xl font-bold hover:opacity-90 active:scale-[0.98] transition-all shadow-lg shadow-primary/30 disabled:opacity-50"
