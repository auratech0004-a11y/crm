...
import { Users, LayoutDashboard, Clock, CalendarCheck, DollarSign, AlertCircle, Settings, LogOut } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeView, onNavigate, isOpen, onClose }) => {
  ...
  const employeeLinks = [
    ...
    { id: 'fines', label: 'My Fines', icon: AlertCircle },
    { id: 'settings', label: 'Settings', icon: Settings },
  ];
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-card border-r border-border z-50 transform transition-transform duration-300 ease-out lg:relative lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 h-full flex flex-col">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h2 className="font-bold text-lg leading-none text-foreground">A.R HRMS</h2>
