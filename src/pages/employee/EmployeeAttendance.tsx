...
import MapAttendance from '@/components/MapAttendance';
    ...
    return (
      <MapAttendance 
        ...
        onComplete={() => {
          ...
          storage.getAttendance().then(att => {
            setRecords(att.filter(a => a.employeeId === user.id));
          });
        }}
    ...
    <div className="space-y-8 animate-slide-up">
      ...
      <div className="bg-card border border-border p-8 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-8 shadow-card">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-xl font-black text-foreground">
              {hasCheckedInToday ? "Already Checked In" : "Today's Attendance"}
            </h2>
        ...
        {!hasCheckedInToday && (
          ...
          >
            <LogIn className="w-5 h-5" /> MARK ATTENDANCE
          </button>
        )}
      </div>
                      ...
                      {openMenuId === r.id && (
                        <>
                          ...
                          <div className="absolute right-6 top-12 z-20 bg-card border border-border rounded-xl shadow-lg py-2 min-w-[180px] dropdown-enter">
                            ...
                            >
                              <AlertCircle className="w-4 h-4 text-warning" /> Submit Appeal
                            </button>
                            <button
                              onClick={() => {
                                setOpenMenuId(null);
                            ...
                            >
                              <AlertCircle className="w-4 h-4 text-warning" /> Appeal
                            </button>
                          </div>
                        </>
                      )}
