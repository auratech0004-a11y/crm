...
const EmployeeManagement = () => {
  ...
  return (
    <div className="space-y-6 animate-slide-up" data-testid="employee-management">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {employees.map((emp) => (
          <div key={emp.id} className="bg-card rounded-3xl border border-border p-5 shadow-card hover:shadow-lg transition-shadow relative group" data-testid={`employee-card-${emp.id}`}>
            <div className="flex items-center gap-4 mb-4">
              <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center text-primary font-bold text-xl overflow-hidden">
                {emp.profile_pic ? (
                  <img src={emp.profile_pic} alt={emp.name} className="w-full h-full object-cover" />
                ) : (
                  emp.name.charAt(0)
                )}
              </div>
              ...
              <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => openEdit(emp)} className="p-2 bg-primary/10 text-primary rounded-lg text-sm hover:bg-primary/20" data-testid={`edit-${emp.id}`}>
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(emp.id, emp.name)} className="p-2 bg-destructive/10 text-destructive rounded-lg text-sm hover:bg-destructive/20" data-testid={`delete-${emp.id}`}>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-border text-sm">
              ...
              <div>
                ...
                <p className="font-bold text-foreground">{emp.joining_date || 'N/A'}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-foreground/60 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-card w-full max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl animate-scale-in border border-border">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 overflow-y-auto max-h-[80vh]" data-testid="employee-form">
              {/* Profile Picture */}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-muted-foreground mb-1 uppercase">Phone</label>
                  <input
                    className="w-full px-4 py-3 bg-secondary border-0 rounded-xl focus:ring-2 focus:ring-primary outline-none text-foreground"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-muted-foreground mb-1 uppercase">Department</label>
                  <input
                    className="w-full px-4 py-3 bg-secondary border-0 rounded-xl focus:ring-2 focus:ring-primary outline-none text-foreground"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-muted-foreground mb-1 uppercase">Role</label>
                  <select
                    className="w-full px-4 py-3 bg-secondary border-0 rounded-xl focus:ring-2 focus:ring-primary outline-none text-foreground"
                    value={formData.role}
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" className="flex-1 py-3.5 gradient-primary text-primary-foreground font-bold rounded-xl shadow-lg shadow-primary/20" data-testid="save-employee-btn">
