...
const EmployeeSalary: React.FC<{ user: Employee }> = ({ user }) => {
  const [isPaid, setIsPaid] = useState(false);

  useEffect(() => {
    const status = storage.getPayrollStatus();
    setIsPaid(status[user.id] === 'Paid');
  }, [user.id]);
    ...
    <div className="space-y-8 animate-slide-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-card border border-border p-8 rounded-3xl flex items-center gap-6 shadow-card">
          ...
          <div>
            <p className="text-4xl font-black text-foreground">₨ {user.salary.toLocaleString()}</p>
      ...
      <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-card">
        ...
        <div className="divide-y divide-border">
          <div className="px-8 py-4 flex items-center justify-between">
            ...
            <span className="font-bold text-foreground">₨ {user.salary.toLocaleString()}</span>
          </div>
          <div className="px-8 py-4 flex items-center justify-between">
          ...
          <div className="px-8 py-4 flex items-center justify-between bg-secondary/30">
            ...
            <span className="font-black text-xl text-primary">₨ {user.salary.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
