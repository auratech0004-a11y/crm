...
const EmployeeLeave = ({ user }) => {
  ...
  const loadLeaves = async () => {
    try {
      const data = await leaveAPI.getAll({ employee_id: user.id });
      setLeaves(data);
    } catch (error) {
  ...
  return (
    <div className="space-y-8 animate-slide-up" data-testid="employee-leave">
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-border">
            {leaves.length === 0 ? (
              ...
              leaves.map((l) => (
                <tr key={l.id} className="hover:bg-secondary/50 transition-colors" data-testid={`leave-record-${l.id}`}>
                  <td className="px-8 py-6 font-bold text-foreground">{l.type}</td>
                  <td className="px-8 py-6 text-foreground">
                    <p className="font-bold text-sm">{l.start_date}</p>
                    <p className="text-[10px] text-muted-foreground font-bold">to {l.end_date}</p>
                  </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-foreground/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-card w-full max-w-md rounded-3xl shadow-2xl border border-border animate-scale-in">
            ...
            <form onSubmit={handleApplyLeave} className="p-6 space-y-4" data-testid="leave-form">
              <div>
              ...
              <button type="submit" className="w-full py-4 bg-warning text-warning-foreground font-black rounded-2xl shadow-xl active:scale-95 transition-all" data-testid="submit-leave-btn">
                SUBMIT REQUEST
              </button>
            </form>
          </div>
