...
const EmployeeDashboard: React.FC<EmployeeDashboardProps> = ({ user, onNavigate }) => {
  const stats = [
    { label: 'Present Days', value: '2.0', sub: 'This month', icon: CheckCircle, color: 'bg-success/10 text-success' },
    { label: 'Absent Days', value: '16.0', sub: 'This month', icon: XCircle, color: 'bg-destructive/10 text-destructive' },
    { label: 'Leave Balance', value: '12', sub: '0 pending', icon: Calendar, color: 'bg-warning/10 text-warning' },
    { label: 'Last Salary', value: '₨ 0', sub: 'Not disbursed', icon: DollarSign, color: 'bg-primary/10 text-primary' },
  ];
  ...
  const leaveSummary = [
    ...
    { label: 'Annual - DC', value: '7.58' },
    { label: 'Sick Leave - DC', value: '2.71' },
    { label: 'Self Wedding - DC', value: '6.56' },
  ];
  ...
  return (
    <div className="space-y-8 animate-slide-up pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-card border border-border p-6 rounded-3xl shadow-card relative overflow-hidden group">
